...
nl_parse_addr4(struct ifaddrmsg *i, int scan, int new)
{
  ...
  if (!a[IFA_ADDRESS])
    {
      log(L_ERR "KIF: Malformed message received (IPV4 missing IFA_ADDRESS)");
      return;
    }
...
nl_parse_addr6(struct ifaddrmsg *i, int scan, int new)
{
  ...
  if (!a[IFA_ADDRESS])
    {
      log(L_ERR "from interface %u:%s",ifi->index,ifi->name);
      log(L_ERR "KIF: Malformed message received (IPV6 missing IFA_ADDRESS)");
      return;
    }
