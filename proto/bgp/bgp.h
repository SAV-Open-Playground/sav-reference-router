...
#define BGP_AF_RPDP4		BGP_AF( BGP_AFI_IPV4, BGP_SAFI_RPDP )
#define BGP_AF_RPDP6		BGP_AF( BGP_AFI_IPV6, BGP_SAFI_RPDP )
...
struct bgp_config {
  ...
  int rr_client;			/* Whether neighbor is RR(Route Reflector) client of me */
  ...
  // the following would be rpdp config
};
...
struct bgp_parse_state {
  ...
  rta *cached_rta;
  uint incoming_interface;
  /* info cache for sav_agent*/
  char as_path[1024];
...
void bgp_get_route_info_char(struct rte *, char *result);
...
#define BA_RPDP4	0xfb	/* rpdp4 flag */
...
#define RPDP_PROTO_NAME "protocol_name"
#endif
