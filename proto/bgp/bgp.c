...
void send_bgp_config(struct bgp_proto *bgp) {
  ...
  BGP_WALK_CHANNELS(bgp, c){
      strcat(channels, ",");
      strcat(channels, c->c.name);
  }
  if (strcmp(channels, "") == 0)
    return;
  // get role;
  char local_role[9] = "";
  char remote_role[9] = "";
  
  switch (bgp->cf->local_role)
  {
  case BGP_ROLE_CUSTOMER:
    strcat(remote_role, "provider");
    strcat(local_role,"customer");
    break;
  case BGP_ROLE_PEER:
    strcat(remote_role, "peer");
    strcat(local_role,"peer");
    break;
  case BGP_ROLE_PROVIDER:
    strcat(remote_role, "customer");
    strcat(local_role,"provider");
    break;
  default:
    strcat(remote_role, "ERROR");
...
bgp_initiate(struct bgp_proto *p)
{
  ...
  if (p->startup_delay)
  {
    ...
    {
      bgp_startup(p);
      /* this function is called if a valid BGP is configured
...
bgp_incoming_connection(sock *sk, uint dummy UNUSED)
{
   ...
   * connections are rejected instead. The exception is the case where an
